...
import (
	...
	"context"
	"fmt"
	"github.com/thanos-community/obslytics/pkg/input/remoteread"
...
func TestThanos_Parquet_e2e_storeapi(t *testing.T) {
	...
	t.Run("export up metric to parquet file using storeapi", func(t *testing.T) {
...
func TestProm_Parquet_e2e_remoteread(t *testing.T) {
	t.Parallel()
	...
	t.Run("export up metric to parquet file using remoteread", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
		defer cancel()

		api, err := remoteread.NewRemoteReadInput(logger, input.InputConfig{
